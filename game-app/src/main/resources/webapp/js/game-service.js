gameApp
    ...
    .service('messageService', ['$resource', function ($resource) {
        this.getMessage = function (img, user, text) {
            var gmList = $resource("templates/data/messages-notifications.json");
    ...
    .service('bestsellingService', ['$resource', function ($resource) {
        this.getBestselling = function (img, name, range) {
            var gbList = $resource("templates/data/best-selling.json");
    ...
    .service('todoService', ['$resource', function ($resource) {
        this.getTodo = function (todo) {
            var todoList = $resource("templates/data/todo.json");
    ...
    .service('recentitemService', ['$resource', function ($resource) {
        this.getRecentitem = function (id, name, price) {
            var recentitemList = $resource("templates/data/recent-items.json");
    ...
    .service('recentpostService', ['$resource', function ($resource) {
        this.getRecentpost = function (img, user, text) {
            var recentpostList = $resource("templates/data/messages-notifications.json");
    ...
    .service('growlCenterService', function () {
        var gs = {};
        gs.growl = function (message, type) {
            $.growl({
                message: message
    ...
    .service('webSocketService', function ($rootScope, $log) {
        var service = {};
        var stompClients = [];

        var getClientId = function (key) {
            var id = _.findIndex(stompClients, function (c) {
                return c.key === key;
            });
        ...
        service.reg = function (key) {
            if (getClientId(key) >= 0) {
                return;
            }

            var websocketListenUrl = "/websocket/listen";

            var socket = new SockJS(GAME_URL_DASHBOARD + websocketListenUrl);
            ...
            stompClient.connect({}, function (frame) {
                ...
                var simpleBroker = WEBSOCKET_SIMPLE_BROCKER + "/" + key;
                stompClient.subscribe(simpleBroker, function (msg) {
                    var data = {
                        "k": key,
                        "v": msg.body
                    }

                    $rootScope.$broadcast(WEBSOCKET_BROADCAST, data);
                });
                stompClient.send(WEBSOCKET_APP_PREFIX + websocketListenUrl, {}, key);
            });
            stompClients.push(
                {
                    key: key,
        ...
        service.unreg = function (key) {
            var id = getClientId(key);
            if (id == -1) {
                return;
            }
            var holder = stompClients.splice(id, 1);
            if (holder && holder.length > 0) {
                var stompClient = holder[0].client;
                stompClient.disconnect(function () {
                    $log.debug("Stomp client disconnected: " + stompClient.key);
                });
            }
        };
